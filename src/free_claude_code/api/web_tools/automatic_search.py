...
async def stream_automatic_web_search_response(
    ...
    *,
    ...
    chunks: list[str] = []
    received_bytes = 0
    # A stall is bounded by the executor's provider-progress deadline (504);
    # the byte cap bounds a runaway decision.
    try:
        async for chunk in provider_stream:
            ...
            if received_bytes > _MAX_SELECTION_STREAM_BYTES:
                raise _protocol_failure(
                    "Upstream model exceeded the automatic WebSearch selection limit.",
                    request_id=request_id,
                )
            chunks.append(chunk)
    finally:

...
def _client_for(
    ...
    *,
    ...
    if progress_timeout is not None:
        # The field is alias-bound (env-only by name); build it through the
        # model validator the app itself uses rather than a dropped kwarg.
        settings = (settings or Settings(_env_file=None)).model_copy(
            update={"provider_progress_timeout": progress_timeout}
        )

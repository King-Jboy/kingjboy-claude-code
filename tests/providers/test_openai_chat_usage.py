...
def test_nested_usage_int_reads_nested_dicts_and_objects():
    assert (
        nested_usage_int(
    ...
    assert (
        nested_usage_int(
            SimpleNamespace(prompt_tokens_details=SimpleNamespace(cached_tokens=15)),
            "prompt_tokens_details",
            "cached_tokens",
        )
        == 15
    ...
    assert (
        nested_usage_int(
            SimpleNamespace(
                model_extra={"prompt_tokens_details": {"cached_tokens": 15}}
            ),
            "prompt_tokens_details",
            "cached_tokens",
        )

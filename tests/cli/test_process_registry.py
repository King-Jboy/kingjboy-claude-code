...
def test_process_registry_force_kill_pid_tree_posix_escalates_process_group(
    monkeypatch,
):
    from free_claude_code.cli import process_registry as pr
    ...
    killpg = patch("os.killpg", create=True)
    with getpgid as mocked_getpgid, killpg as mocked_killpg:
        pr.force_kill_pid_tree_best_effort(12345)

    mocked_getpgid.assert_called_once_with(12345)
    mocked_killpg.assert_called_once_with(
        5678, getattr(pr.signal, "SIGKILL", pr.signal.SIGTERM)
    )
